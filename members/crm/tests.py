...
class MembershipApplicationTest(TestCase):
    ...
    def testFormSubmission(self):
        ...
        self.assertIn(
            "New Membership Application: %s" % app.display_name, email.subject
        )
...
class TestLoginKey(TestCase):
    ...
    def testRequestLoginKey(self):
        ...
        response = self.client.post(
            "/", {"email": "wrong@example.com", "organization": 1}
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(
            response, "E-mail you entered is not associated with selected organization"
        )

        response = self.client.post(
            "", {"email": "tech@oeglobal.org", "organization": 1}
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "E-mail sent")

        login_key = LoginKey.objects.latest("id")
        self.assertEqual(login_key.email, "tech@oeglobal.org")
...
class BillingLogTest(LiveServerTestCase):
    ...
    def setUp(self):
        ...
        self.user, is_created = User.objects.get_or_create(
            username="staff_member", is_staff=True, is_superuser=True, is_active=True
        )
    ...
    def testCreateInvoice(self):
        ...
        self.assertRedirects(
            response, reverse("staff:organization-view", kwargs={"pk": org.id})
        )
        ...
        self.assertIn("tech@oeglobal.org", email.bcc)
        self.assertIn("Custom body", email.body)

        data.update({"log_type": "create_paid_invoice"})
        response = self.client.post(reverse("staff:billinglog-create"), data)
        self.assertRedirects(
            response, reverse("staff:organization-view", kwargs={"pk": org.id})
        )
