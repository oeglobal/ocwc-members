...
class VoteView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["proposition_votes"] = PropositionBallot.objects.filter(
            organization=org, election=context["election"]
        ).order_by("proposition__title")
        context["candidate_votes"] = CandidateBallot.objects.filter(
            organization=org, election=context["election"]
        )

        if (
            self.object.propositionballot_set.filter(organization=org).count()
            < org.get_number_of_votes()
        ):
...
class VoteAddFormView(LoginRequiredMixin, FormView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if self.election.candidateballot_set.filter(
            seat_type="institutional", organization=self.organization
        ).count():
            return redirect(
                reverse("elections:vote-view", kwargs={"pk": self.election.id})
            )
